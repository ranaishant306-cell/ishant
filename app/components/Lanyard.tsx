...
'use client';
// Rapier's imperative RigidBody refs (with the ad-hoc `.lerped` velocity-smoothing
// field bolted on at runtime) and the GLTF's untyped nodes/materials don't have
// precise upstream types, so this file uses `any` at those specific boundaries
// rather than fighting three.js/rapier's internals with fragile casts.
        ...
        <Physics gravity={gravity} timeStep={isMobile ? 1 / 30 : 1 / 60}>
          {cards.map((c, i) => (
            <Band
              ...
              lanyardImage={c.lanyardImage}
              lanyardWidth={c.lanyardWidth || 1}
              anchorX={i * 4 - (cards.length - 1) * 2}
            />
          ))}
        </Physics>
        <Environment blur={0.75}>
...
function Band({
  ...
  useEffect(() => {
    // Three.js textures are imperative objects, not React state — setting wrap
    // mode here is the standard r3f pattern, not a render-purity violation.
